...
### サブエージェントは `survey` だけ自分の判断で呼ぶ

**既定は「明示指示があるときだけ」。** その例外が `survey`（列挙と分類）1 本で、**こちらの判断で起動してよい**。

| | 自律起動 | なぜ |
| --- | --- | --- |
| **`survey`** | **してよい** | 読み取り専用で、規約もフックも効く。**探し方とコマンドと件数を必ず返す契約**があるので検算できる |
| `Explore` / `Plan` | しない | **CLAUDE.md と rules を読まない仕様**。規約を知らない相手が判断することになる |
| `general-purpose` ほか | しない | 既定のまま |

**使うのは「読む量が多く、結論が短く、結論だけで検算できる」作業だけ。** 数えるだけなら `grep` で足り、判断の根拠として原文を読む必要がある作業には使わない。
