...
**会話へ出すときは、3バッククォートの `mermaid` フェンスを、さらに4バッククォートで囲む。** 3バッククォートだけだと TUI がフェンスを描画して**マーカーを剥がす**ので、コピーしても `mermaid` タグが付いてきません。**貼り先で図として描画されない形**になります。

**上の規定は貼り先での要件で、こちらは渡す経路の要件です。** 開きと閉じを正しく書いても、剥がれた後の本文が渡ります。

**記法で終わらせず、図として描いて見せる。** TUI は Mermaid を描かないので、フェンスだけでは読み手に**コードが残ります**。`mermaid` の MCP がある環境では、ブラウザへ描くところまでやる。

**フェンスの書式とは別の経路です。** 上の2つは**貼り先へ渡すため**、こちらは**その場で見せるため**。図を主張の根拠に使うなら、**描いた結果を見てから出す**（ `concise_first_then_detail.md` の「描画結果を見る」）。
