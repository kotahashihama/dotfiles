...
**囲み方は、その図を何に使うかで決まります。** 画面で見せるのと、貼り先へ渡すのは別の目的で、両立しません。

| 目的 | 書き方 | 起きること |
| --- | --- | --- |
| **その場で見せる** | **3バッククォートの `mermaid` フェンス** | TUI がその場で図へ置き換える |
| **貼り先へ渡す**（読み手が Notion や GitHub へコピーする） | **さらに4バッククォートで囲む** | ソースのまま残り、`mermaid` タグごとコピーできる |

**囲まないとソースが取れません。** 3バッククォートだけだと画面上は図になりますが、**コピーで取れるのも図のほう**なので、貼り先で描画されない形になります。

**既定は3バッククォート。** 会話の読み手はその場で読むので、渡す必要があるときだけ4バッククォートにする。
