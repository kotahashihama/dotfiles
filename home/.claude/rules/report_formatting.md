...
ユーザーへの報告にコミットハッシュや URL を書くときは、**受け取った側がコピーやクリックでそのまま使える形**にしてください。レビュー対応の一覧と Mermaid の図を会話へ出すときの書式も、本ルールが持ちます。
これは全プロジェクト共通の規約です。
...
## Mermaid は、図とソースの両方を出す

開きに `mermaid` を付け、末尾を閉じます。閉じを落とすと後続の文章まで飲まれます。使うかどうかの判断は `concise_first_then_detail.md` が持ちます。

TUI は描画に成功するとソースを捨てるので、**同じソースを、情報文字列を付けない4バッククォートで囲んだものも並べる。** 外側に `mermaid` と書くと、2つとも図になります。図が先、ソースが後。
...
描画には実測した癖があります。**交差する辺は畳まれて判読できず、線が別のノードを貫通することがある。** どちらも正しい絵に見えるので、描画結果を見てから出す。幅が足りないと図が出ず、`gantt` は描けず、`subgraph` は横並びになる。`sequenceDiagram` と `classDiagram` は強く、`flowchart` は交差と合流に弱い。辺が交差する構造は、表か箇条書きにする。
...
- `github_writing.md` （GitHub 上で他リポジトリの PR / issue を参照する書式）: あちらは GitHub へ書く文章、本ルールはユーザーへの報告が対象
