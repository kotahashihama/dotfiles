...
**なぜ**: 受け取った側は PR を開いて**上から順に返信していく**。並びが違うと1件ごとに表の中を探し直すことになり、件数が増えるほどコストが積み上がります。一覧を出すのは突き合わせを楽にするためなので、並べ替えるとその目的自体が失われます。
...
**図とソースを両方出す。** TUI は描画に成功すると**ソースを捨てて図だけ**にするので、3バッククォートで書いただけでは原文が残りません。

| 書くもの | 囲み | 何が出るか |
| --- | --- | --- |
| **1つ目** | 3バッククォートの `mermaid` フェンス | **図**。TUI が置き換える |
| **2つ目**（同じソース） | **情報文字列を付けない4バッククォート**で、3バッククォートの `mermaid` フェンスを**中に入れる** | **ソース**。描画されずに残る |

**4バッククォート側に `mermaid` と書かない。** 判定は**開いたフェンスの情報文字列**なので、そこに書くと外側が描画対象になり、**2つとも図になります**（実際に踏んだ）。外側は空にする。
...
**順は図が先、ソースが後。** 読み手は図を見に来ており、**怪しいときだけ原文へ下りる**。

**ソースを残すのは、描画が発展途上だからです。** 線が別のノードを貫通する等の癖があり（下記）、**図だけ出すと読み手が誤りを確かめられません**。原文があれば、図と突き合わせて判断できる。

**コピーの経路も兼ねます。** 4バッククォートで囲んだほうは `mermaid` タグごとコピーできるので、読み手が Notion や GitHub へ貼れば、そちらの描画器で描き直せます。

**図が1つも要らないなら、ソースだけで構いません。** 両方出すのは、図を主張の根拠に使うときです。

**描いた結果を読んでから出す。** 構文が通っても、意図した形になるとは限りません（ `concise_first_then_detail.md` の「描画結果を見る」）。実測で分かっている癖は次の3つ。

| 癖 | 実測 | 重さ |
| --- | --- | --- |
| **交差する辺が畳まれる** | 2対2で引いた4本が**2本の横線になり**、どれがどこへ繋がるか読めない | **最も重い** |
| **線が別のノードを貫通する** | `修正中 --> レビュー中` の線がマージ済の箱を横切り、**出どころがマージ済に見えた** | 中 |
...
| `subgraph` は横並びになる | 前と後の対比は**別のブロックに分ける**（ `concise_first_then_detail.md` ） | 軽い |

**下2つは気づけますが、上2つは正しい絵に見えます。** だから描いた結果を読む必要があります。
...
**辺が交差する構造は、図にしない。** 表か箇条書きへ倒す。**図にできるのは、木に近い形だけ**だと考える。
