...
| 癖 | 実測 |
| --- | --- |
| **線が別のノードを貫通する** | `修正中 --> レビュー中` の線がマージ済の箱を横切り、**出どころがマージ済に見えた** |
| **幅が足りないと図が出ない** | `not rendered: wider than N columns` の1行だけが返る。折り返しはしない |
| `subgraph` は横並びになる | 前と後の対比は**別のブロックに分ける**（ `concise_first_then_detail.md` ） |

**種類で出来が違います。** `sequenceDiagram` は同期と返りを線種で描き分けて最も読みやすく、`erDiagram` は分岐を持たせても縦の鎖になります。
