...
数値と単位のあいだの空白や全角の約物の後ろの空白は、既成の日本語向け
lint が見ない。書く場所ごとに別々の実装を置くと判定がずれるので、判定は
...
# 1つの語の中で英字と日本語が混ざっているなら、その語ごと前後の日本語と詰める
# （ `応募意思はJR東日本も` ）。語の内側の扱いが外側を決めるので、英字を独立
# させたなら両側を空ける（ `Aurora が` ）。判定は英字の直後が日本語かどうか。
# 範囲の `〜` は区切りの記号で語の一部ではないため、JA から外す（ `は v2〜v3` ）
MIXED = r'[A-Za-z][A-Za-z0-9]*[ぁ-んァ-ヶ一-龥々]'

CHECKS = [
    ('混在した語と日本語', re.compile(JA + r' ' + MIXED)),
