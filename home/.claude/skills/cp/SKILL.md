...
6. **フックが重い場合**（全テストを同期実行する等でコミットが数分ブロックする）:
   - **飛ばす前に、そのフックが何を見ているか読む。** `--no-verify` は**1つのフラグで全部のフックを飛ばす**ので、避けたい重い検査だけでなく、**テスト以外を見るもの**（固有名の走査・秘匿値の検出）まで一緒に消える
     - **読む対象は設定ではなく中身。** 入口はリポジトリごとに違い（ `core.hooksPath` を指すもの、lefthook が `.git/hooks` へ書くもの）、設定を見ただけでは「フックが無い」と誤読する
     - **ステージした範囲で検査対象が変わるものがある**（公開側のファイルだけを走査する等）。フックの名前からは分からない
   - **同じ検証を自分で先に済ませてから**フックを飛ばす。土台は `background_heavy_work.md`
   - 順序は「**フックの中身を読む** → **見ているものを自分で流す** → 通ったことを確認する → `--no-verify` でコミット」。**流す対象は読んだ結果で決まる**ので、テストと lint に固定しない
...
- 通常時: 変更ファイル数と増減行数を1行 + **push したコミットを1行ずつ列挙**（レンジ表記は使わない）
- 変更なし: 「変更なし」の1行で終わる
...
- **検証せずに** `--no-verify` 等でフックを回避すること（同じ検証を自分で済ませた場合のみ可。手順6）
- **フックの中身を読まずに `--no-verify` を打つこと**。1つのフラグで全部飛ぶので、**避けたい検査とは別のもの**（固有名の走査・秘匿値の検出）まで消える。「テストは通した」は代替になっていない
