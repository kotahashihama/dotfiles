...
### 7. 対応したコメントへリアクションを付ける

**コードで応えたコメントに `+1`（👍）を付ける**（`react_to_addressed_reviews.md`）。返信しない運用のため、**対応済みであることを伝える唯一の手段**になる。

```bash
gh api -X POST repos/OWNER/REPO/pulls/comments/<comment_id>/reactions -f content='+1'
```

非対応・`[ask]`・コード変更を伴わないものには付けない。
...
| 動詞 | 差分の性質で選ぶ（`対応しました` / `修正しました` / `取り込みました`） |
| 生成者表示の Note | **付けない**。ユーザーの言葉として投稿するため（`github_note_generated_by_claude.md` の例外） |

**返信するのはコミットを伴うものだけ。** ハッシュを添えられないコメントは投稿せず、**返信案として会話で渡す**。

| コメント | 扱い |
| --- | --- |
| コードを直した | **返信する**。ハッシュを添える |
| **`[ask]` の質問** | **返信しない**。答えはユーザーの言葉で返すもの |
| **コード変更を伴わない**（PR 説明の誤りの指摘、非対応の判断） | **返信しない**。返信案を渡す |

**指摘と違う判断をしたなら、ハッシュの後に理由を 1〜2 行足す。** 黙って別の形で直すと、レビュアーは指摘どおりに直ったと読む。
