...
層の定義と判断表は `docs/layers.md`。取り込みの実体は `scripts/adopt_dotfile.sh` が持つので、**手で `mv` + `ln` しない**。
...
3. **層を決める**: `docs/layers.md` の判断表に従う。**迷ったら private に倒す**
...
8. **検証する**: `scripts/lib.sh` を変更したなら `/verify-restore`、公開層へ入れたなら `/audit-secrets` を走らせる
...
判定の根拠（管理する理由・層を選んだ理由）を 1 行ずつ添え、取り込み後のリンクを**リンク元 → リンク先**で示す。秘匿値を切り出した場合は変数名だけを挙げる（値は出さない）。
...
- 取り込んだ設定ファイルに「これは private です」等の**層の説明を書き足す**こと（`managed-file-purity.md`）
